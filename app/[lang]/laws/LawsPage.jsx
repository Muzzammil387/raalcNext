...
const LawsPage = ({ lang }) => {
    const { loading, data } = useFetch(`faqlaw/law/${lang}`);
    const [lawsActive, setLawsActive] = useState("")
                                    ...
                                    <div className="section6MainBox_ !bg-transparent border-b py-2 border-[#ddd]">
                                        <div className="section6MainBoxc w-full">
                                            <div className="h4  cursor-pointer accordions py-2  grid grid-cols-[7fr,3fr] " onClick={() => handleTab(index)}><span className="leading-[1.1] font-Mluvka">{question}</span>
                                                <Image src={arrow} className={`ml-auto  w-[1.438rem] relative chevron transition-all ease-in-out duration-300 ${lawsActive === index ? "rotate-180" : ""}`} width={10} height={10} alt="" />
                                            </div>
                                            <div className={`section6MainBoxcDOx overflow-hidden transition-all ease-in-out duration-300 ${lawsActive === index ? "max-h-max" : "max-h-0"}`}>
